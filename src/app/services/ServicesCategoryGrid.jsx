...
import { categoryServiceHref } from "@/app/lib/categoryRoutes";
  ...
  if (!categories?.length) {
    return (
      <div className="services_cat_empty rounded-3 p-4 text-center">
        ...
        <Link href="/compnies" className="btn services_cat_cta_btn">
          Browse listings
        </Link>
      </div>
    );
  ...
  return (
    <nav className="services_cat_strip" aria-label="Service categories">
      {categories.map((cat) => {
        ...
        const href = categoryServiceHref(cat) || "/compnies";
